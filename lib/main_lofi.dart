...
class _ZenFlowLoFiMainScreenState extends State<ZenFlowLoFiMainScreen> {
  ...
  final List<Color> _accentPalette = const [
    Colors.green,
    Colors.yellow,
    Colors.red,
  ];
  ...
  void _addHabit(String title, String frequency) {
    ...
    final color = _accentPalette[app.habits.length % _accentPalette.length];
    app.addHabit(title.trim(), frequency, color.toARGB32());
  }
...
class LoFiHabitsScreen extends StatelessWidget {
  ...
  final List<HabitItem> habits;
  final void Function(int index) onToggleHabit;
  final void Function(String title, String frequency) onAddHabit;
  final void Function(
    int index,
    String? title,
    String? frequency,
  ...
  Widget _buildLoFiHabitItem(BuildContext context, int index, HabitItem habit) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border.all(
          color: accent.withAlpha((0.6 * 255).toInt()),
          width: 1.5,
        ),
        borderRadius: BorderRadius.circular(4),
      ),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              onAddHabit(controller.text, frequency);
  ...
  Widget _accentChoice(Color color, bool selected, VoidCallback onSelect) {
    return GestureDetector(
      onTap: onSelect,
      child: Container(
        width: 28,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          ElevatedButton(
            onPressed: () => _showActionMessage('avatar editor opened'),
